...
# 탭 구성
user_tab, purchase_tab, product_tab, region_tab, revisit_tab = st.tabs([
    "유저 분석", "구매 분석", "제품 분석", "지역/유입 분석", "복귀 유저 분석"
])
...
# churn_rates와 cc_list는 months[:-1]의 길이와 같으므로, months[:-1] 기준으로 계산
months_for_churn = months[:-1] if len(months) > len(churn_rates) else months
months_for_cc = months[:-1] if len(months) > len(cc_list) else months
highlight_months = (
    [months_for_churn[i] for i in np.concatenate([high_churn_idx, low_churn_idx])]
    if len(months_for_churn) and (len(high_churn_idx) > 0 or len(low_churn_idx) > 0)
    else []
)

fig = go.Figure()
fig.add_bar(
    x=[str(m) for m in months_for_cc],
    y=cc_list,
    name="Carrying Capacity",
    marker_color=main_red
)
# monthly_mau를 months_for_cc에 맞춰서 reindex
if isinstance(monthly_mau, pd.Series) and not monthly_mau.empty:
    mau_values = monthly_mau.reindex(months_for_cc, fill_value=0).values
else:
    mau_values = []
fig.add_scatter(
    x=[str(m) for m in months_for_cc],
...
# CC 하이라이트 마커(파란색/노란색, y축 동일)
if len(high_cc_idx) > 0:
    fig.add_scatter(
        x=[str(months_for_cc[i]) for i in high_cc_idx],
...
if len(low_cc_idx) > 0:
    fig.add_scatter(
        x=[str(months_for_cc[i]) for i in low_cc_idx],
...
fig.update_layout(
    title="월별 Carrying Capacity & MAU (단일 y축)",
    xaxis_title="월",
    yaxis_title="값",
    height=400,
    hovermode='x unified'
...
fig_churn.add_trace(go.Scatter(
    x=[str(m) for m in months_for_churn],
    y=churn_rates,
    name="이탈률(%)",
...
if len(high_churn_idx) > 0:
    fig_churn.add_scatter(
        x=[str(months_for_churn[i]) for i in high_churn_idx],
        y=[churn_rates[i] for i in high_churn_idx],
        mode='markers',
        marker=dict(color=highlight, size=8),
        name='Churn High'
    )
if len(low_churn_idx) > 0:
    fig_churn.add_scatter(
        x=[str(months_for_churn[i]) for i in low_churn_idx],
...
with user_tab:
    ...
    # --- 유저 분석 탭의 맨 아래에 추가 ---
    # 전체 기간 기준 이탈 유저 집합 구하기
    filtered_events['year_month'] = filtered_events['created_at'].dt.to_period('M')
    months = sorted(filtered_events['year_month'].unique())
    churned_users_all = set()
    for i, month in enumerate(months[:-1]):
        this_month_users = set(filtered_events[filtered_events['year_month'] == month]['user_id'])
        next_month = months[i+1]
        next_month_users = set(filtered_events[filtered_events['year_month'] == next_month]['user_id'])
        churned = this_month_users - next_month_users
        churned_users_all.update(churned)
    # 이탈 유저들의 country 집계
    churned_profiles = filtered_users[filtered_users['id'].isin(churned_users_all)]
    country_churn_count = churned_profiles['country'].value_counts().sort_values(ascending=False)
    # 전체 country별 유저수
    total_country_counts = filtered_users['country'].value_counts()
    # country별 이탈률 계산
    country_churn_rate = (country_churn_count / total_country_counts * 100).fillna(0).sort_values(ascending=False)
    fig_state_churn_rate = go.Figure()
    fig_state_churn_rate.add_bar(
        x=country_churn_rate.index.astype(str),
        y=country_churn_rate.values,
        marker_color='rgb(255,99,132)'
    )
    fig_state_churn_rate.update_layout(
        title="국가별 이탈률 (%)",
        xaxis_title="국가",
        yaxis_title="이탈률 (%)",
        height=500
    )
    st.plotly_chart(fig_state_churn_rate, use_container_width=True, key="user_tab_state_churn_rate")

    # --- 국가별 신규 가입자 수 그래프 추가 ---
    country_new_users = filtered_users.groupby('country')['id'].nunique().sort_values(ascending=False)
    import plotly.graph_objects as go
    fig_country_new_users = go.Figure()
    fig_country_new_users.add_bar(
        x=country_new_users.index.astype(str),
        y=country_new_users.values,
        marker_color='rgb(0,123,255)'
    )
    fig_country_new_users.update_layout(
        title="국가별 신규 가입자 수",
        xaxis_title="국가",
        yaxis_title="신규 가입자 수",
        height=400
    )
    st.plotly_chart(fig_country_new_users, use_container_width=True, key="user_tab_country_new_users")

    # 월별 MAU 변화율 계산
    mau_values = monthly_mau.values
    mau_pct_change = pd.Series(mau_values).pct_change() * 100
    mau_pct_change.index = monthly_mau.index

    fig_mau_change = go.Figure()
    fig_mau_change.add_bar(
        x=[str(m) for m in mau_pct_change.index],
        y=mau_pct_change.values,
        marker_color='rgb(0,200,83)'
    )
    fig_mau_change.update_layout(
        title="전월 대비 MAU 변화율 (%)",
        xaxis_title="월",
        yaxis_title="MAU 변화율 (%)",
        height=300
    )
    st.plotly_chart(fig_mau_change, use_container_width=True, key="user_tab_mau_change")

    # --- 월별 MAU, 신규 유저 수, 복귀 유저 수 그래프 추가 ---
    # 월별 신규 유저 수
    monthly_new_users = filtered_users.groupby('year_month')['id'].nunique()
    # 월별 MAU
    monthly_mau = filtered_events.groupby('year_month')['user_id'].nunique()
    # 월별 복귀 유저 수 = MAU - 신규 유저 수
    monthly_returning_users = monthly_mau - monthly_new_users
    monthly_returning_users = monthly_returning_users.clip(lower=0)  # 음수 방지
    fig_mau_new_return = go.Figure()
    fig_mau_new_return.add_bar(
        x=[str(m) for m in monthly_mau.index],
        y=monthly_new_users.reindex(monthly_mau.index, fill_value=0).values,
        name="신규 유저수",
        marker_color=main_red
    )
    fig_mau_new_return.add_bar(
        x=[str(m) for m in monthly_mau.index],
        y=monthly_returning_users.values,
        name="복귀 유저수",
        marker_color=mid_red
    )
    fig_mau_new_return.add_scatter(
        x=[str(m) for m in monthly_mau.index],
        y=monthly_mau.values,
        name="MAU",
        mode="lines+markers",
        line=dict(color=light_red, width=3)
    )
    fig_mau_new_return.update_layout(
        title="월별 MAU, 신규 유저 수, 복귀 유저 수",
        xaxis_title="월",
        yaxis_title="유저 수",
        barmode="stack",
        height=400
    )
    st.plotly_chart(fig_mau_new_return, use_container_width=True, key="user_tab_mau_new_return")
...
with region_tab:
    ...
    with col1:
        ...
        st.plotly_chart(fig_traffic_bar, use_container_width=True, key="region_tab_traffic_bar")
    with col2:
        # --- 복귀 이력 테이블(revisit_df) 생성 및 활용 ---
        filtered_events['year_month'] = filtered_events['created_at'].dt.to_period('M')
        user_months = filtered_events.groupby('user_id')['year_month'].apply(list)
        revisit_records = []
        for user_id, ym_list in user_months.items():
            ym_list = sorted([pd.Period(m) for m in ym_list])
            for i in range(len(ym_list)-1):
                if ym_list[i+1] > ym_list[i]+1:
                    revisit_records.append({'user_id': user_id, '복귀월': ym_list[i+1]})
        import pandas as pd
        revisit_df = pd.DataFrame(revisit_records)
        # 복귀월의 첫 이벤트 traffic_source 집계
        if not revisit_df.empty:
            events['year_month'] = events['created_at'].dt.to_period('M')
            revisit_df['user_id'] = revisit_df['user_id'].astype(int)
            revisit_df['복귀월'] = revisit_df['복귀월'].astype(str)
            events['user_id'] = events['user_id'].astype(int)
            events['year_month'] = events['year_month'].astype(str)
            revisit_events = pd.merge(
                revisit_df,
                events,
                left_on=['user_id', '복귀월'],
                right_on=['user_id', 'year_month'],
                how='left'
            )
            revisit_events = revisit_events.sort_values(['user_id', 'created_at']).drop_duplicates(['user_id', '복귀월'])
            traffic_counts_revisit = revisit_events['traffic_source'].value_counts().sort_values(ascending=False)
            if not traffic_counts_revisit.empty:
                import plotly.express as px
                fig_revisit_traffic = px.pie(
                    names=traffic_counts_revisit.index,
                    values=traffic_counts_revisit.values,
                    title='복귀 유저의 유입 경로별 유저 수',
                    color_discrete_sequence=['rgb(220,53,69)', 'rgb(255,99,132)', 'rgb(255,179,186)', '#ffcccc', '#ff9999', '#ff6666', '#ff3333', '#ffb3b3', '#ff4d4d'][:len(traffic_counts_revisit)]
                )
                fig_revisit_traffic.update_layout(height=350)
    ...
    st.plotly_chart(fig_state_churn_rate, use_container_width=True, key="region_tab_state_churn_rate")

with revisit_tab:
    st.header("복귀 유저 분석")
    # 복귀 이력 테이블(revisit_df) 생성 (region_tab col2와 동일)
    filtered_events['year_month'] = filtered_events['created_at'].dt.to_period('M')
    user_months = filtered_events.groupby('user_id')['year_month'].apply(list)
    revisit_records = []
    for user_id, ym_list in user_months.items():
    ...
    revisit_df = pd.DataFrame(revisit_records)
    # 복귀 유저 id 집합 (int로 변환)
    revisit_ids = set(revisit_df['user_id'].astype(int))
    revisit_profiles = users[users['id'].astype(int).isin(revisit_ids)]
    profile_colors = [main_red, mid_red, light_red, highlight]
    st.subheader('복귀 유저 프로파일링')
    # 1. 성별 분포 (users 기준)
    if 'gender' in revisit_profiles.columns:
        counts = revisit_profiles['gender'].value_counts()
        fig_profile = go.Figure()
        fig_profile.add_bar(
            x=counts.index.astype(str),
            y=counts.values,
            marker_color=profile_colors[0 % len(profile_colors)]
        )
        fig_profile.update_layout(
            title="복귀 유저 성별 분포",
            xaxis_title="성별",
        ...
        st.plotly_chart(fig_profile, use_container_width=True)
    # 2. 연령대 분포 (users 기준)
    if 'age' in revisit_profiles.columns:
        ages = pd.to_numeric(revisit_profiles['age'], errors='coerce').dropna().astype(int)
        ...
        fig_profile.add_bar(
            ...
            y=age_counts.values,
            marker_color=profile_colors[1 % len(profile_colors)]
        )
        fig_profile.update_layout(
            title="복귀 유저 연령대 분포",
            xaxis_title="연령대",
        ...
        st.plotly_chart(fig_profile, use_container_width=True)
    # 3. 국가 분포 (users 기준)
    if 'country' in revisit_profiles.columns:
        counts = revisit_profiles['country'].value_counts().head(10)
        fig_profile = go.Figure()
        fig_profile.add_bar(
            x=counts.index.astype(str),
            y=counts.values,
            marker_color=profile_colors[2 % len(profile_colors)]
        )
        fig_profile.update_layout(
            title="복귀 유저 국가 Top 10",
            xaxis_title="국가",
            yaxis_title="유저 수",
            height=300
        )
        st.plotly_chart(fig_profile, use_container_width=True)
    # 4. 유입경로 분포 (복귀월 첫 이벤트 기준)
    if not revisit_df.empty:
        events['year_month'] = events['created_at'].dt.to_period('M')
        revisit_df['user_id'] = revisit_df['user_id'].astype(int)
        revisit_df['복귀월'] = revisit_df['복귀월'].astype(str)
        ...
        revisit_events = revisit_events.sort_values(['user_id', 'created_at']).drop_duplicates(['user_id', '복귀월'])
        if 'traffic_source' in revisit_events.columns:
            counts = revisit_events['traffic_source'].value_counts()
            fig_profile = go.Figure()
            fig_profile.add_bar(
                x=counts.index.astype(str),
                y=counts.values,
                marker_color=profile_colors[3 % len(profile_colors)]
            )
            fig_profile.update_layout(
                title="복귀 유저 유입경로 분포 (복귀월 첫 이벤트 기준)",
                xaxis_title="유입경로",
                yaxis_title="유저 수",
                height=300
            )
    ...
    # 5. 복귀월 기준 복귀 유저의 구매 카테고리 분포
    if not revisit_df.empty:
        # events, order_items, products 데이터 준비
        events['year_month'] = events['created_at'].dt.to_period('M')
        revisit_df['user_id'] = revisit_df['user_id'].astype(int)
        revisit_df['복귀월'] = revisit_df['복귀월'].astype(str)
        events['user_id'] = events['user_id'].astype(int)
        ...
        if 'products' not in st.session_state:
            ...
            products = st.session_state['products']
        order_items['user_id'] = order_items['user_id'].astype(int)
        ...
        revisit_purchases = pd.merge(
            revisit_purchases,
            order_items[['user_id', 'product_id', 'year_month']],
            left_on=['user_id', '복귀월'],
            right_on=['user_id', 'year_month'],
            how='left'
        )
        products['id'] = products['id'].astype(str)
        revisit_purchases['product_id'] = revisit_purchases['product_id'].astype(str)
        revisit_purchases = pd.merge(
            revisit_purchases,
        ...
        category_counts = revisit_purchases['category'].value_counts().sort_values(ascending=False)
        if not category_counts.empty:
            ...
            fig.update_layout(
                title='복귀월 기준 복귀 유저의 구매 카테고리 분포',
                xaxis_title='카테고리',
                yaxis_title='구매 건수',
                height=350
            )
            ...
            st.info('복귀월 기준 복귀 유저의 구매 카테고리 데이터가 없습니다.')

    # 6. 복귀 유저 중 중국 유저의 복귀월 기준 구매 상품 Top 10
    if not revisit_df.empty:
        # 중국 유저 id 추출
        cn_ids = set(revisit_profiles[revisit_profiles['country'] == 'China']['id'].astype(int))
        # 복귀월에 구매한 이벤트만 추출
        cn_revisit_df = revisit_df[revisit_df['user_id'].isin(cn_ids)]
        events['year_month'] = events['created_at'].dt.to_period('M')
        events['user_id'] = events['user_id'].astype(int)
        events['year_month'] = events['year_month'].astype(str)
        cn_revisit_df['user_id'] = cn_revisit_df['user_id'].astype(int)
        cn_revisit_df['복귀월'] = cn_revisit_df['복귀월'].astype(str)
        cn_revisit_purchases = pd.merge(
            cn_revisit_df,
            events[(events['event_type'] == 'purchase')],
            left_on=['user_id', '복귀월'],
            right_on=['user_id', 'year_month'],
        ...
        products['id'] = products['id'].astype(str)
        cn_revisit_purchases['product_id'] = cn_revisit_purchases['product_id'].astype(str)
        cn_revisit_purchases = pd.merge(
            cn_revisit_purchases,
            products[['id', 'name']],
            left_on='product_id',
        ...
        product_counts = cn_revisit_purchases['name'].value_counts().sort_values(ascending=False).head(10)
        if not product_counts.empty:
            ...
            fig.update_layout(
                title='복귀 유저(중국) 복귀월 기준 구매 상품 Top 10',
                xaxis_title='구매 건수',
                yaxis_title='상품명',
                height=400
            )
            ...
            st.info('중국 복귀 유저의 복귀월 기준 구매 상품 데이터가 없습니다.')

    # 7. 복귀 유저의 재이탈률/재복귀율
    if not revisit_df.empty:
        # 복귀월 이후 다음 월에 활동한 적이 없는 유저(재이탈)
        revisit_df['복귀월_period'] = pd.PeriodIndex(revisit_df['복귀월'], freq='M')
        user_months_dict = user_months.to_dict()
        re_leave = 0
        re_return = 0
        for user_id, 복귀월 in zip(revisit_df['user_id'], revisit_df['복귀월_period']):
            ym_list = sorted([pd.Period(m) for m in user_months_dict.get(user_id, [])])
            # 복귀월 이후 다음 월에 활동한 적이 있는지
            after = [m for m in ym_list if m > 복귀월]
            if after:
                # 재복귀: 복귀월 이후 또다시 이탈 후 복귀가 있는지
                re_revisit = False
                ...
                re_leave += 1
        total = len(revisit_df)
        re_leave_rate = re_leave / total * 100 if total > 0 else 0
        re_return_rate = re_return / total * 100 if total > 0 else 0
        # 스코어카드 한 줄에 배치
        scorecards = []
        # 신규 유저 이탈률/복귀 비율 먼저 append
        if not filtered_users.empty and not filtered_events.empty:
            # 가입월, 가입 다음 달 계산
            filtered_users['가입월'] = filtered_users['created_at'].dt.to_period('M')
            filtered_users['가입다음월'] = filtered_users['가입월'] + 1
            # 가입 다음 달에 활동한 유저 id 집합
            filtered_events['year_month'] = filtered_events['created_at'].dt.to_period('M')
            next_month_activity = filtered_events.groupby('year_month')['user_id'].unique().to_dict()
            left_users = 0
            total_new = 0
            for idx, row in filtered_users.iterrows():
                uid = row['id']
                next_month = row['가입다음월']
                if pd.isna(next_month):
                    continue
                total_new += 1
                # 가입 다음 달에 활동한 적이 없으면 이탈로 간주
                if uid not in next_month_activity.get(next_month, []):
                    left_users += 1
            leave_rate = left_users / total_new * 100 if total_new > 0 else 0
            scorecards.append({
                'label': '신규 유저 이탈률',
                'value': f'{leave_rate:.1f}%',
                'help': '가입 다음 달에 활동하지 않은 비율'
            })
            # 복귀 비율
            filtered_users['가입월'] = filtered_users['created_at'].dt.to_period('M')
            filtered_users['가입다음월'] = filtered_users['가입월'] + 1
            filtered_events['year_month'] = filtered_events['created_at'].dt.to_period('M')
            month_user_dict = filtered_events.groupby('year_month')['user_id'].unique().to_dict()
            revisit_count = 0
            total_new = 0
            for idx, row in filtered_users.iterrows():
                uid = row['id']
                ...
                if pd.isna(next_month):
                    continue
                total_new += 1
                # 가입 다음 달에 활동 안 한 경우만 복귀 여부 체크
                if uid not in month_user_dict.get(next_month, []):
                    # 이후 월에 다시 활동한 적이 있는지
                    after_months = [m for m in month_user_dict.keys() if m > next_month]
                    found = False
                    for m in after_months:
                        if uid in month_user_dict[m]:
                            revisit_count += 1
                            found = True
                            break
            revisit_rate = revisit_count / total_new * 100 if total_new > 0 else 0
            scorecards.append({
                'label': '신규 유저 복귀 비율',
                'value': f'{revisit_rate:.1f}%',
                'help': '가입 다음 달 이탈 후 이후 월에 다시 활동한 비율'
            })
        # 복귀 유저 재이탈률/재복귀율 append
        if not revisit_df.empty:
            scorecards.append({
                'label': '복귀 유저 재이탈률',
                'value': f'{re_leave_rate:.1f}%',
                'help': '복귀 후 다음 월에 활동하지 않은 비율'
            })
            scorecards.append({
                'label': '복귀 유저 재복귀율',
                'value': f'{re_return_rate:.1f}%',
                'help': '복귀 후 또 복귀한 비율'
            })
    if scorecards:
        ...
        st.info('신규 유저 복귀 비율을 계산할 데이터가 없습니다.')